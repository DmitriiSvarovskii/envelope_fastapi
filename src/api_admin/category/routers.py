...
async def get_all_category(store_id: int, current_user: User = Depends(get_current_user_from_token), session: AsyncSession = Depends(get_async_session)):
    try:
        categories = await crud_get_all_categories(schema=current_user.username, store_id=store_id, session=session)
        return categories
...
async def create_new_category(store_id: int, data: CategoryCreate,  current_user: User = Depends(get_current_user_from_token), session: AsyncSession = Depends(get_async_session)):
    try:
        new_category = await crud_create_new_category(schema=current_user.username, store_id=store_id,  data=data, user_id=current_user.id, session=session)
        return new_category
    except Exception as e:
        await session.rollback()
        raise HTTPException(
