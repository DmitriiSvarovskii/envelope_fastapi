...
from .schemas import (
    ...
    UnitList, UnitCreate, UnitUpdate
)
...
async def crud_delete_product(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Категория, c id {product_id}, успешно удалена."
        }
    except IntegrityError:
        raise HTTPException(
            status_code=400,
            detail=(
                "Удаление этой категории невозможно, "
...
async def crud_update_product_field(
    schema: str,
    product_id: int,
    user_id: int,
    checkbox: str,
...
async def crud_create_new_unit(
    schema: str,
    data: UnitCreate,
    user_id: int,
    session: AsyncSession = Depends(get_async_session)
...
async def crud_delete_unit(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Категория, c id {unit_id}, успешно удалена."
        }
    except IntegrityError:
        raise HTTPException(
            status_code=400,
