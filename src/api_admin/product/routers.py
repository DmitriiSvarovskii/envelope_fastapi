...
from .schemas import ProductList, ProductCreate, ProductUpdate
from .crud import (
    crud_create_new_product,
    crud_update_product,
    crud_update_product_field,
    crud_change_delete_flag_product,
    crud_delete_product,
)
...
async def create_new_product(
    ...
    try:
        new_product = await crud_create_new_product(
...
#     object_key = (
    # f"{current_user.id}/{store_id}/{current_date_str}_{file.filename"
    # )
...
async def update_product(
    ...
    try:
        up_product = await crud_update_product(
            schema=str(current_user.id),
...
async def update_product_field(
    ...
    """
    try:
        change_product = await crud_update_product_field(
            schema=str(current_user.id),
            user_id=current_user.id,
            product_id=product_id,
            checkbox=checkbox,
...
@router.patch("/delete/")
async def change_delete_flag_product(
    product_id: int,
    current_user: User = Depends(get_current_user_from_token),
    session: AsyncSession = Depends(get_async_session)
):
    try:
        change_product = await crud_change_delete_flag_product(
            schema=str(current_user.id),
            user_id=current_user.id,
            product_id=product_id,
            session=session
...
async def delete_product(
    ...
    try:
        change_product = await crud_delete_product(
            schema=str(current_user.id),
            product_id=product_id,
            session=session
        )
...
# # # query_id%3DAAGVbSskAAAAAJVtKyQX6XyE&user
# %3D%257B%2522id%2522%253A606825877%252C%2522first_name
# %2522%253A%2522%25D0%2594%25D0%25BC%25D0%25B8%25D1%2582%25D1
# %2580%25D0%25B8%25D0%25B9%2522%252C%2522last_name%2522
# %253A%2522%25D0%25A1%25D0%25B2%25D0
# %25B0%25D1%2580%25D0%25BE%25D0%25B2%25D1
# %2581%25D0%25BA%25D0%25B8%25D0%25B9%2522%252C%2522username
# %2522%253A%2522swarovskidima%2522%252C%2522language_code%2522%253A
# %2522ru%2522%252C%2522is_premium%2522
# %253Atrue%252C%2522allows_write_to_pm%2522%253Atrue%257D&auth_date%3D1696961678&hash%3Db0c8bb03576e9ecf85171fe151bee2f996dec619353ce4a2bce72d6337f24678
...
# #         # Здесь вы можете проверить, что токен
# действителен и содержит нужную информацию о пользователе
