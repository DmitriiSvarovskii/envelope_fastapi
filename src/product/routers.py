...
async def create_new_product(access_token: Annotated[str, Depends(apikey_scheme)], new_product: ProductCreate, session: AsyncSession = Depends(get_async_session)):
...
@router.put("/products/")
async def update_product(access_token: Annotated[str, Depends(apikey_scheme)], product_id: int, new_date: ProductUpdate, session: AsyncSession = Depends(get_async_session)):
...
async def delete_product(access_token: Annotated[str, Depends(apikey_scheme)], shop_id: int, product_id: int, session: Session = Depends(get_async_session)):
    try:
